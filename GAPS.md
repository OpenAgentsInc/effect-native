...
| Canvas scene renderer | Khala fleet board and gym panes need typed graph/timeline scenes over three-effect | 2026-07-08 | shipped -> #22 (`@effect-native/render-canvas`: closed scene-descriptor catalog `effect-native/scene/v1`, pure scene reconciler, Scope/`Stream` frame loop, headless backend, **live** three-effect-shaped graph via `makeLiveThreeSceneGraph` / `makeLiveThreeEffectCanvasBackend` — `createSceneNodeReconciler` API + mesh/line/points/label/group catalogue, Scope-bridged disposal, optional WebGL draw; depends on `three`. First catalog consumer GraphFigure #37. Apps may still inject a custom `ThreeSceneGraph` port) |
