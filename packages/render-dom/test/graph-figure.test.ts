    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const view = (revision: number): View =>
            SplitPane({
              ...
              panes: [
                {
                  id: "main",
                  content: Stack({ key: "root", direction: "row" }, [
                    GraphFigure({
                      key: "graph",
                      layout: "precomputed",
                      width: 320,
          ...
          expect(updatedTimeline?.querySelector('[data-en-event="ev1"] [data-en-role="label"]')?.textContent).toBe(
            "Pairing opened 1"
          )
