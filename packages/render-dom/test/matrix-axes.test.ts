      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const field = container.querySelector(
            '[data-en-key="field-legacy"] [data-en-role="control"]'
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const surface = yield* makeDomRenderer({ document }).mount(container, Stream.make(view), noopReport)
          const field = container.querySelector(
            '[data-en-key="field-matrix"] [data-en-role="control"]'
          ) as HTMLInputElement | null
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          yield* makeDomRenderer({ document }).mount(container, Stream.make(view), noopReport)
          const field = container.querySelector(
            '[data-en-key="field-invalid"] [data-en-role="control"]'
          ) as HTMLInputElement | null
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          yield* makeDomRenderer({ document }).mount(container, Stream.make(view), noopReport)
          const field = container.querySelector(
            '[data-en-key="field-gutter"] [data-en-role="control"]'
          ) as HTMLInputElement | null
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          yield* makeDomRenderer({ document }).mount(container, Stream.make(view), noopReport)
          const field = container.querySelector(
            '[data-en-key="field-autoresize"] [data-en-role="control"]'
...
describe("Select/SelectControl trigger matrix axes (#79) DOM rendering", () => {
  ...
  test("multi-select toggles selected option membership and fires onChange with the next values array", async () => {
    ...
    const report: IntentReporter = (_ref, payload) =>
      Effect.sync(() => {
