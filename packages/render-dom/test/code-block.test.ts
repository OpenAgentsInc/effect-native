...
import { CodeBlock, DiffView, IntentRef, Stack, makeViewProgramFromState, type IntentReporter, type View } from "@effect-native/core"
...
describe("code block + diff (#36) DOM renderer", () => {
  test("tokenized code, copy, unified diff, review verdicts, and source-control actions", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
          ...
          DiffView({
            ...
            hunks: [{
              ...
              rows: [
                ...
                { kind: "add", newLine: 5, id: "r-2", verdict: "pending", tokens: [{ kind: "plain", text: "return 2" }] }
              ]
            }]
          })
        ])
      ...
      ;(code?.querySelector('[data-en-role="copy"]') as HTMLElement | null)
      ...
      ;(addRow?.querySelector('[data-en-verdict-action="approved"]') as HTMLElement | null)
        ?.dispatchEvent(new window.MouseEvent("click", { bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(verdicts).toEqual([{ rowId: "r-2", verdict: "approved" }])

      // per-line comment dispatches { rowId }
      ;(addRow?.querySelector('[data-en-role="comment-action"]') as HTMLElement | null)
        ?.dispatchEvent(new window.MouseEvent("click", { bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(comments).toEqual([{ rowId: "r-2" }])

      // source-control action dispatches the action id
      ;(diff?.querySelector('[data-en-action="approve"]') as HTMLElement | null)
        ?.dispatchEvent(new window.MouseEvent("click", { bubbles: true }) as unknown as Event)
      yield* nextTask
