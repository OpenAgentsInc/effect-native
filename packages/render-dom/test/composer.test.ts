...
import { Combobox, Composer, IntentRef, makeViewProgramFromState, type IntentReporter, type View } from "@effect-native/core"
...
describe("composer (#32) DOM renderer", () => {
  test("contenteditable model, key commands, paste normalization, autocomplete", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      ...
      editor?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "Enter", bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(commands).toContain("submit")
      expect(submits).toEqual(["Ship @Orrery"])

      commands.length = 0
      editor?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "Enter", shiftKey: true, bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(commands).toEqual(["newline"])

      // ArrowUp/ArrowDown are history commands
      commands.length = 0
      editor?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowUp", bubbles: true }) as unknown as Event)
      editor?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowDown", bubbles: true }) as unknown as Event)
      yield* nextTask
