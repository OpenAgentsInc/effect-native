...
describe("DOM interaction algebra (#24)", () => {
  ...
  test("keyboard-driven listbox selection updates aria-activedescendant end to end", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      ...
      listbox().dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowDown", bubbles: true }) as unknown as Event)
      yield* nextTask
      ...
      listbox().dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowUp", bubbles: true }) as unknown as Event)
      yield* nextTask
      yield* Effect.yieldNow
  ...
  test("modifier-specific key bindings only fire on an exact modifier match", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      ...
      composer.dispatchEvent(new window.KeyboardEvent("keydown", { key: "Enter", shiftKey: true, bubbles: true }) as unknown as Event)
      composer.dispatchEvent(new window.KeyboardEvent("keydown", { key: "Enter", metaKey: true, bubbles: true }) as unknown as Event)
      yield* nextTask
  ...
  test("file drop yields a typed dropped-items payload with no closures in the tree", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      ...
      const program = makeViewProgramFromState(state, view)
      const report: IntentReporter = (ref, runtimeValue) => Effect.sync(() => {
