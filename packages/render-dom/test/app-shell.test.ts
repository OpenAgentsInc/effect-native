    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          expect((bench?.querySelector('[data-en-pane="chat"]') as HTMLElement | null)?.textContent).toContain(
            "Chat pane"
          )
          ...
          yield* nextTask
          expect(
            (container.querySelector('[data-en-key="rail-nav"] [data-en-section="panes"]') as HTMLElement | null)
              ?.scrollTop
          ).toBe(48)
  ...
  test("controlled nav selection stays visible across root commits", async () => {
    ...
    Object.defineProperty(htmlElementPrototype, "clientHeight", {
      ...
      get(this: HTMLElement) {
        return this.hasAttribute("data-en-section") ? 32 : 0
      }
    })
    try {
      await Effect.runPromise(
