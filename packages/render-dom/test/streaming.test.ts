...
describe("DOM live streaming (#26)", () => {
  test("appended stream items reuse existing keyed DOM elements", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      const region = yield* makeStreamRegion(Stream.fromIterable<RegionPatch<string>>([
        { _tag: "Append", items: [{ key: "line-0", value: "first" }] }
      ]), { frameMillis: 4 })

      const program = makeViewProgramFromState(
        region.items,
        (items): View =>
          Stack({ key: "transcript", direction: "column" }, items.map((item) =>
            Text({ key: item.key, content: item.value, variant: "body" })))
      )
