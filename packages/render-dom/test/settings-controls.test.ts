      ...
      const view = (): View =>
        Stack({ key: "settings", direction: "column" }, [
          FieldRow({
            ...
            error: "1–32 workers",
            controlKey: "workers",
            control: NumberField({ key: "workers", value: 8, min: 1, max: 32, step: 1, invalid: true, onChange: { name: "Workers" } })
          }),
          Toggle({ key: "auto", value: true, label: "Auto-approve", onChange: { name: "Auto" } }),
          Select({ key: "model", value: "claude", label: "Model", disabled: false, onChange: { name: "Model" }, options: [
            { value: "claude", label: "Claude" },
            { value: "codex", label: "Codex" }
          ] }),
          Checkbox({ key: "stream", checked: false, label: "Stream", onChange: { name: "Stream" } }),
          RadioGroup({ key: "mode", name: "mode", value: "review", onChange: { name: "Mode" }, options: [
