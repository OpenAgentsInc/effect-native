...
describe("tabs (#30) DOM renderer", () => {
  test("tablist a11y, keyboard nav, and panel association by id", async () => {
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function*() {
      ...
      chatTab?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowRight", bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(selected).toEqual(["editor"])

      // ArrowLeft from chat wraps to the last enabled tab (editor; terminal disabled)
      selected.length = 0
      chatTab?.dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowLeft", bubbles: true }) as unknown as Event)
      yield* nextTask
      expect(selected).toEqual(["editor"])
