...
const commands: ReadonlyArray<CommandDefinition> = [
  {
    ...
    group: "General",
    scope: "global",
    binding: { key: "p", meta: true },
    intent: IntentRef("OpenPalette", StaticPayload(null))
  },
  {
    id: "thread.next",
    title: "Next thread",
    ...
    scope: "global",
    binding: { key: "Escape" },
    intent: IntentRef("NextThread", StaticPayload(null))
  },
  // Same chord (Escape) in a higher scope shadows the global binding.
  {
    ...
    scope: "palette-open",
    binding: { key: "Escape" },
    intent: IntentRef("CloseOverlay", StaticPayload(null))
  },
  // Enablement gated by a context flag.
  {
    ...
    group: "Composer",
    scope: "composer",
    binding: { key: "Enter", meta: true },
    when: "composer.dirty",
    intent: IntentRef("SubmitComposer", StaticPayload(null))
  }
]
...
describe("keymap / keybinding registry (#41)", () => {
  ...
  test("a recorded key stream fires the right command in the right scope", async () => {
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        const handlers: IntentHandlers<typeof definitions> = {
          OpenPalette: () =>
            Effect.sync(() => {
              fired.push("OpenPalette")
            }),
          CloseOverlay: () =>
            Effect.sync(() => {
              fired.push("CloseOverlay")
            }),
          NextThread: () =>
            Effect.sync(() => {
              fired.push("NextThread")
            }),
          SubmitComposer: () =>
            Effect.sync(() => {
              fired.push("SubmitComposer")
            })
        }
        ...
        const press = (chord: KeyChord) => Effect.provideService(keymap.dispatchChord(chord), IntentRegistry, registry)
    ...
    expect(result.fired).toEqual(["OpenPalette", "NextThread", "CloseOverlay", "NextThread", "SubmitComposer"])
  })
})
