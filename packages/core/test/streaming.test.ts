        ...
        Effect.gen(function* () {
          ...
          const program = makeViewProgramFromState(
            ...
            (current): View =>
              List(
                { key: "transcript" },
                current.map((item) => keyed(Text({ key: item.key, content: item.value, variant: "body" })))
              )
          )
  ...
  test("closing the region scope interrupts the source stream and releases resources", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const region = yield* Scope.provide(scope)(makeStreamRegion(Stream.fromQueue(queue), { frameMillis: 4 }))
