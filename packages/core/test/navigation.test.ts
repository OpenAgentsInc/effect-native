...
const runNavigationSequence = (sequence: ReadonlyArray<Intent<string, JsonPayload>>) =>
  Effect.gen(function* () {
    ...
    const intentLayer = makeNavigationIntentRegistryLayer({ now: () => 1 }).pipe(Layer.provide(navigationLayer))
...
describe("typed navigation intent", () => {
  test("Navigate dispatch records typed destinations and replays deterministically", async () => {
    ...
    const replayed = await Effect.runPromise(runNavigationSequence(first.events.map((event) => event.intent)))

    expect(first.recorded).toEqual(destinations)
    expect(replayed.recorded).toEqual(first.recorded)
    expect(first.events.map((event) => event.intent.name)).toEqual(["Navigate", "Navigate", "Navigate"])
