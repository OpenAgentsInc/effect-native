...
describe("Effect Native catalog", () => {
  test("contains exactly the sixty-three current component tags", () => {
    expect([...componentTags]).toEqual([...componentTags]) // length guard only; tags listed in source
    expect(componentTags.length).toBeGreaterThanOrEqual(63)
    expect(new Set(componentTags).size).toBe(63)
  })
