...
describe("ViewProgram runtime and headless renderer", () => {
  ...
  test("same initial state and intent sequence produce byte-identical snapshots", async () => {
    const first = await Effect.runPromise(
      Effect.map(runCounter([2, 4, 8]), ({ snapshots }) => JSON.stringify(snapshots))
    )
  ...
  test("headless renderer cleanup is tied to Scope finalizers", async () => {
    const finalized = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const surface = yield* renderer.mount({ onFinalize: Ref.set(probe, true) }, program.viewStream, () =>
  ...
  test("state-bound Text updates through a binding path without re-specifying the tree", async () => {
    const snapshots = await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const program = yield* makeViewProgram({ profile: { name: "Ada" } }, () => tree)
          const renderer = makeHeadlessRenderer()
          const surface = yield* renderer.mount(undefined, program.viewStream, program.report)
    ...
    expect(snapshots).toEqual([Text({ content: "Ada", variant: "body" }), Text({ content: "Grace", variant: "body" })])
  })
})
